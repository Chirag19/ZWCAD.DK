...
class ZcIfstream : public
  ...
  public:
    class ZcIfstream() {};
    class ZcIfstream(const ZCHAR *pName);
#ifdef UNICODE
...
class ZcOfstream : public
  ...
  public:
    class ZcOfstream() {};
    class ZcOfstream(const ZCHAR *pName);
#ifdef UNICODE
    void open(const wchar_t *pName);
    void open(const wchar_t *pName, ios_base::openmode nMode);
#endif
...
#ifndef UNICODE
inline ZcIfstream::ZcIfstream(const ZCHAR *pName) : std::ifstream(pName)
{
}
inline ZcOfstream::ZcOfstream(const ZCHAR *pName) : std::ofstream(pName)
{
}
#else
inline ZcIfstream::ZcIfstream(const ZCHAR *pName)
{
    this->open(pName);
}

inline ZcOfstream::ZcOfstream(const ZCHAR *pName)
{
    this->open(pName);
}
