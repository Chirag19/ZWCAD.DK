...
class ZcString : public ZcHeapOperators
{
  public:
    ...
    int find(ZCHAR ch) const;

    int find(const ZCHAR *psz) const;

    int find(const ZcString & acs) const;

    int findOneOf(const ZCHAR *psz) const;

    int findRev(ZCHAR ch) const;

    int findRev(const ZCHAR *psz) const;

    int findOneOfRev(const ZCHAR *psz) const;
    ...
    void formatV(const ZCHAR   *pszFmt,  va_list args);
    ...
    ZcString precat(ZCHAR ch) const;
...
#ifdef UNICODE
inline const ZCHAR * ZcString::kZCharPtr() const
{
    return this->kwszPtr();
}
#else
...
inline ZcString operator + (ZCHAR ch, const ZcString & acs)
{
