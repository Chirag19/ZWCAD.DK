...
#define IDR_ZSDKPROMPTBASE	700

BOOL zcedSetIUnknownForCurrentCommand (const LPUNKNOWN) ;

struct ZPROMPT_MAP_ENTRY {
	LPCWSTR m_szDesc ;
	DISPID m_dispId ;
	int m_promptType ;
	int m_cronly ;
...
protected:
	...
	DECLARE_REGISTRY_RESOURCEID(ZIDR_ZSDKPROMPTBASE) 
	...
	int GetPromptMapSize () {
		ZPROMPT_MAP_ENTRY *pPromptMap =GetPromptMap () ;
		int i =0 ;
		while ( pPromptMap [i].m_szDesc )
			i++ ;
		return (i) ;
	}
	virtual ZPROMPT_MAP_ENTRY *GetPromptMap () =0 ;
...
#define BEGIN_PROMPT_MAP(CLASSNAME) \
	...
	virtual ZPROMPT_MAP_ENTRY* GetPromptMap () { \
		static ZPROMPT_MAP_ENTRY pPromptMap [] = {
