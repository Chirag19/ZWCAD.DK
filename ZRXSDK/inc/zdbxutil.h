...
Zcad::ErrorStatus zcdbGetProxyInfo(ZcDbObject *pObj, ZCHAR*& dxfName,
                                   ZCHAR*& className, ZCHAR*& appName);
...
bool zcdbGetPreviewBitmapFromDwg(const ZCHAR* pszDwgfilename,
                                 HBITMAP* pPreviewBmp,
                                 HPALETTE* pRetPal);

const ZCHAR* zcdbGetReservedString(ZcDb::reservedStringEnumType reservedType,
    bool bGetLocalized = true);
const ZCHAR* zcdbXlateReservedString(const ZCHAR* strSource,
    bool bGetLocalized = true);
bool zcdbIsReservedString(const ZCHAR* strString,
...
bool ZDESK_STDCALL zcdbDwkFileExists(const ZCHAR *pszFileName);


int ZDESK_STDCALL zcdbQueueForRegen(const ZcDbObjectId *pIdArray,
                                    int nNumIds);
