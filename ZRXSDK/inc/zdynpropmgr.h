...
struct _OPM_DYNPROP_ENTRY {
    ...
    const ZCHAR *strCmd ;
    ZcRxClass *pClass ;
    IZDynamicProperty *pProp ;
    const CLSID* pclsid ;
    IZPropertySource *pPropSrc ;
} ;
...
#define OPM_DYNPROP_OBJECT_ENTRY_AUTO(classname, rxclass) \
    OPM_DYNPROP_OBJECT_ENTRY_MAKE(classname, rxclass, __uuidof (IZDynamicProperty2))

#define OPM_DYNPROP_OBJECT_LEGACY1ENTRY_AUTO(classname, rxclass) \
    OPM_DYNPROP_OBJECT_ENTRY_MAKE(classname, rxclass, __uuidof (IZDynamicProperty))
...
#define OPM_DYNPROP_COMMAND_ENTRY_AUTO(classname, cmdname) \
    OPM_DYNPROP_COMMAND_ENTRY_MAKE(classname, cmdname, __uuidof (IZDynamicProperty2))

#define OPM_DYNPROP_COMMAND_LEGACY1ENTRY_AUTO(classname, cmdname) \
    OPM_DYNPROP_COMMAND_ENTRY_MAKE(classname, cmdname, __uuidof (IZDynamicProperty))


#define OPM_DYNPROP_PERINSTANCE_ENTRY_AUTO(classname, rxclass) \
    OPM_DYNPROP_OBJECT_ENTRY_MAKE(classname, rxclass, __uuidof (IZPropertySource))
...
public:
    ZcRxDynPropManager () {
        ...
        for ( _OPM_DYNPROP_ENTRY **ppEntry =m_ppOpmObjMapEntryFirst ; ppEntry < m_ppOpmObjMapEntryLast ; ppEntry++ ) {
            if ( *ppEntry != NULL ) {
                if ( *((*ppEntry)->pclsid) == __uuidof (IZDynamicProperty2) || *((*ppEntry)->pclsid) == __uuidof (IZDynamicProperty) ) {
                    CComPtr<IZPropertyManager> pPropMan ;
                    ...
                    if ( (*ppEntry)->pClass != NULL && (*ppEntry)->pClass == ZcDbDatabase::desc () ) {
                        ...
                        ZOPMPropertyExtension *pOPMExt =GET_OPMEXTENSION_CREATE_PROTOCOL ()->CreateOPMCommandProtocol ((*ppEntry)->strCmd) ;
                ...
                } else if ( *((*ppEntry)->pclsid) == __uuidof (IZPropertySource) ) {
					...
					IZPropertySource *pPropSrc =GET_OPM_PERINSTANCE_PROPERTY_SOURCES ()->GetPropertySourceAt (&propName) ;
					if ( pPropSrc == NULL ) {
